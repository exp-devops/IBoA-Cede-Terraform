...
# Additional EKS access entry for qacededevelopereks IAM user
resource "aws_eks_access_entry" "qacededevelopereks_user" {
  cluster_name  = aws_eks_cluster.main.name
  principal_arn = "arn:aws:iam::228886154405:user/qacededevelopereks"
  type          = "STANDARD"
}

# Additional EKS access policy association for qacededevelopereks IAM user
resource "aws_eks_access_policy_association" "qacededevelopereks_user" {
  cluster_name  = aws_eks_cluster.main.name
  ...
  depends_on = [aws_eks_access_entry.qacededevelopereks_user]
}
